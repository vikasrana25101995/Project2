...
public class computer extends SQLiteOpenHelper
{
    ...
    private static final String Database_path = "/data/data/com.example.rohin.myapplication3/databases/";
    ...
    private void createDB() {
        ...
        Log.d("CHECK2 Boolean",Boolean.toString(dbexist));
        ...
        Log.d("Database Already Exist","Already Exist");
    }

    private void copyDBfromResource()
    {
        Log.d("CHECKING123","INSIDE COPY RESOURCE");
        InputStream is;
        OutputStream os;
        String filePath = Database_path+Database_name;
        Log.d("CHECKING123","SET FILE PATH");
        try
        {
            is = context.getAssets().open(Database_name);
            Log.d("CHECKING123","Open Assets DAtabase");
            os = new FileOutputStream(filePath);//writing purpose
            Log.d("CHECKING123","SET THE DTABASE IN MOBILE");
            byte[] buffer = new byte[1024];
            int length;
            while ((length = is.read(buffer)) > 0)
            {
                Log.d("READ BUFFER",Integer.toString(length));
